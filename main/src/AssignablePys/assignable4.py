...
def assignable4():
    ...
    with open("userinfo//Assignables//path4.txt", "r") as file:
       path = file.read()
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED:
            break
        if event == "APPLY":
            with open("userinfo//Assignables//path4.txt", "w") as file:
                file.write(values["IN"])
            with open("userinfo//Assignables//name4.txt", "w") as file:
                file.write(values['NAME'])    
                
        if event == "WEBSITE":
            with open("userinfo//Assignables//assign4.txt", "w") as file:
                file.write("Website\n")
                file.close()
