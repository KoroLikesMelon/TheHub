...
def assignable2():
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED:
            break
        if event == "APPLY":
            with open("userinfo//Assignables//path2.txt", "w") as file:
                file.write(values["IN"])
            with open("userinfo//Assignables//name2.txt", "w") as file:
                file.write(values['NAME'])    
                
        if event == "WEBSITE":
            with open("userinfo//Assignables//assign2.txt", "w") as file:
                file.write("Website\n")
                file.close()
